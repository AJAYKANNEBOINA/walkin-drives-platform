...
const ForEmployers = () => (
  <section className="py-12 sm:py-20">
    <div className="container">
      <motion.div
        className="rounded-2xl sm:rounded-3xl border border-border bg-card p-6 sm:p-10 md:p-16"
      ...
      >
        <div className="mx-auto max-w-2xl text-center">
          ...
          <p className="mb-8 sm:mb-10 text-sm sm:text-base text-muted-foreground leading-relaxed">
            Post your walk-in drive and reach local candidates instantly. No waiting, no delays – find the right talent today.
          </p>

          <div className="mb-8 sm:mb-10 grid grid-cols-2 gap-3 sm:gap-4 md:grid-cols-4">
            {perks.map(p => (
              <div key={p.title} className="rounded-xl bg-secondary p-3 sm:p-4 text-center">
                <p.icon className="mx-auto mb-2 h-5 w-5 text-primary" />
                <p className="text-xs sm:text-sm font-semibold text-foreground">{p.title}</p>
          ...
          <Link to="/signup">
            <Button size="lg" className="rounded-full px-6 sm:px-8 font-semibold text-sm sm:text-base">
