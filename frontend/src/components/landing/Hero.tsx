...
const Hero = () => {
  ...
  return (
    <section className="relative overflow-hidden">
      ...
      <motion.div
        className="pointer-events-none absolute -right-32 top-10 h-[300px] w-[300px] md:h-[500px] md:w-[500px] rounded-full bg-[hsl(var(--purple))]/20 blur-[100px]"
        animate={{ x: [0, 30, -20, 0], y: [0, -40, 20, 0], scale: [1, 1.15, 0.9, 1], opacity: [0.2, 0.35, 0.15, 0.2] }}
        transition={{ duration: 8, repeat: Infinity, ease: "easeInOut" }}
      />
      <motion.div
        className="pointer-events-none absolute -left-40 bottom-0 h-[250px] w-[250px] md:h-[400px] md:w-[400px] rounded-full bg-primary/15 blur-[100px]"
        animate={{ x: [0, -20, 30, 0], y: [0, 30, -20, 0], scale: [1, 0.9, 1.2, 1], opacity: [0.15, 0.3, 0.1, 0.15] }}
        ...
        <div className="grid items-center gap-8 lg:grid-cols-2 lg:gap-8">
          ...
          <div className="relative z-10 text-center lg:text-left">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
              <span className="mb-4 inline-flex items-center gap-2 rounded-full border border-primary/20 bg-primary-light px-3 py-1 text-[11px] sm:text-xs font-semibold text-primary sm:px-4 sm:py-1.5">
            ...
            <motion.h1
              className="mb-4 sm:mb-6 text-3xl sm:text-4xl md:text-5xl lg:text-[3.75rem] font-extrabold leading-[1.1] text-foreground"
              initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.1 }}
            >
              Let us make{" "}
              <br className="hidden sm:block" />
              <motion.span className="relative inline-block" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.25 }}>
            ...
            <motion.p
              className="mb-6 sm:mb-8 mx-auto max-w-lg text-sm sm:text-base leading-relaxed text-muted-foreground md:text-lg lg:mx-0"
            ...
            <motion.div
              className="flex flex-wrap items-center justify-center lg:justify-start gap-3 sm:gap-5 text-xs sm:text-sm text-muted-foreground"
          ...
          <div className="relative flex items-center justify-center">
            ...
            {/* Floating latest drives card - hidden on very small screens */}
            <motion.div
              className="absolute -bottom-4 -left-2 z-20 hidden sm:block w-56 md:w-64 rounded-xl border border-border bg-card/95 p-2.5 md:p-3 card-shadow backdrop-blur-sm"
              initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.6 }}
            >
              <div className="mb-2 flex items-center justify-between">
                <span className="text-[10px] md:text-xs font-bold text-foreground">Latest Drives</span>
                <span className="flex items-center gap-1 rounded-full bg-[hsl(var(--mint))]/10 px-2 py-0.5 text-[9px] md:text-[10px] font-semibold text-[hsl(var(--mint))]">
            ...
            {/* Floating stats card */}
            <motion.div
              className="absolute -right-1 top-2 z-20 hidden sm:block rounded-xl border border-border bg-card/95 px-3 py-2 md:px-4 md:py-3 card-shadow backdrop-blur-sm"
              initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5, delay: 0.8 }}
              whileHover={{ scale: 1.05 }}
            >
              <p className="text-base md:text-lg font-extrabold text-foreground">50K+</p>
              <p className="text-[9px] md:text-[10px] text-muted-foreground">Active Job Seekers</p>
            </motion.div>
          </div>
        </div>
      </div>
